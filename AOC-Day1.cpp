...
private:
	...
	void assertEqual(std::string actual, std::string expected, const char* message) {
		if (actual == expected) {
			std::cout << GREEN << "[PASS] " << RESET << message << std::endl;
		}
		else {
			std::cerr << RED << "[FAIL] " << RESET << message << " (Expected: " << expected << ", Actual: " << actual << ")" << std::endl;
		}
	}

	void assertEqualInt(int actual, int expected, const char* message) {
		if (actual == expected) {
			std::cout << GREEN << "[PASS] " << RESET << message << std::endl;
		}
		else {
...
#pragma region String test cases
	...
	void testNoDigit() {
		...
		assertEqual(actual, "0", "Test string - no digit");
	}
...
int main() {
	std::cout << AdventOfCodeDay1().day1() << std::endl;
	AdventOfCodeDay1().unitTest();
}